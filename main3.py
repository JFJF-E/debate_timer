...
def exit_check():
    ...
    if(state!=11 and (ticks1>maxtick or ticks2>maxtick)):
...
state = 1
state_list = ['','左方一辩','右方一辩','左二质询','左方二辩','右二质询','右方二辩','左三质询','左方三辩','右三质询','右方三辩','自由辩论','右方四辩','左方四辩']
ticks_list=[]
warn0 = 0
warn1 =   0
warn2= 0
...
for i in range(len(ticks_list)):
    ...
    while(exit_check()):  #每一次循环时就检查软件按钮状态
        ...
        elif(state in [2,5,6,9,10,12]):    #2号队伍
